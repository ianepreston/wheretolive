...
PLACES = ["DOWNTOWN", "GF_WORK", "BROTHER", "TO_BC"]
...
def make_select():
    ...
    sql = f"""
    SELECT
    rfaster_id AS rfaster_commute_id,
    {_make_commute_cases()}
    FROM public.rfaster;
     """
    return sql
...
def make_sql():
    ...
    return "\n\n\n".join(
        [
            "DROP VIEW IF EXISTS rfaster_commutes CASCADE;",
            "CREATE OR REPLACE VIEW rfaster_commutes AS",
